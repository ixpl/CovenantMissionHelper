...
local function constructString(unit, isWin)
        local result = unit.name .. L['.'] .. ' ' .. L['HP'] .. ' = ' .. unit.currentHealth .. '/' .. unit.maxBaseHealth .. '\n'
...
function Board:getResultInfo()
    ...
    text = string.format("%s\n%s:\n%s \n\n%s %s %s = %s (%s%s)",
    		warningText, L['My units'], text, L['TOTAL'], loseOrGain, L['HP'], math.abs(lostHP), math.abs(lostHPPerc), "%")
